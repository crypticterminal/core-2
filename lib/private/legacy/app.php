...
class OC_App {
	 ...
	 * TODO we seem to be using the db as a cache for app types. we have those in the appinfo as well. simplify?
	 *
	 * @param $app
	 */
	...
	public static function getAllApps() {
		...
		foreach (\OC::$APPSROOTS as $apps_dir) {
			if (!\is_readable($apps_dir['path'])) {
				\OC::$server->getLogger()->warning("unable to read app folder: ${apps_dir['path']}", ['app'=>__METHOD__]);
				continue;
			}
	...
	public static function listAllApps() {
		...
		foreach ($installedApps as $app) {
			if (!\in_array($app, $blacklist, true)) {
				...
				$info = \OC::$server->getAppManager()->getAppInfo($app);
				if (!\is_array($info)) {
					\OC::$server->getLogger()->error("Could not read app info file for app '$app'", ['app'=>__METHOD__]);
					continue;
				}

				if (!isset($info['name'])) {
					\OC::$server->getLogger()->error("App id '$app' has no name in appinfo", ['app'=>__METHOD__]);
					continue;
				}
	...
	public static function getStorage($appId) {
		if (self::isEnabled($appId)) { //sanity check
			...
			\OC::$server->getLogger()->error("Can't get app storage, app '$appId', user not logged in", ['app'=>__METHOD__]);
			return false;
		}
		\OC::$server->getLogger()->error("Can't get app storage, app '$appId' not enabled", ['app'=>__METHOD__]);
		return false;
	}
